...
var MySQLErrName = map[uint16]string{
	...
	ErrSlaveIgnoredTable:                        "SlaveList SQL thread ignored the query because of replicate-*-table rules",
	...
	ErrSlaveWasRunning:                          "SlaveList is already running",
	ErrSlaveWasNotRunning:                       "SlaveList already has been stopped",
	...
	ErrBinlogRowRbrToSbr:                        "SlaveList running with --log-slave-updates must use row-based binary logging to be able to replicate row-based binary log events",
	...
	ErrSQLthreadWithSecureSlave:                              "Setting authentication options is not possible when only the SlaveList SQL Thread is being started.",
	...
	ErrSlaveConfiguration:                                    "SlaveList is not configured or failed to initialize properly. You must at least set --server-id to enable either a master or a slave. Additional error messages can be found in the MySQL error log.",
	...
	ErrSlaveSilentRetryTransaction:                           "SlaveList must silently retry current transaction",
